...
using POS.Infractructure.Helpers;
...
namespace POS.Infractructure.Persistences.Repositories
{
    public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
    {
        ...
        {
            //Listado de state activos y no eliminados
            var getAll = await _entity
                .Where(x => x.State.Equals((int)StateTypes.Active) && x.AuditDeleteUser == null 
        ...
        public async Task<bool> RemoveAsync(int id)
        {
            ...
            //y devuelvo los registros afectados
            var recordsAffected = await _context.SaveChangesAsync();
            return recordsAffected > 0;
        }
        ...
        //Esto nos va a servir sobre todo para la lista, el listado de datos para un par de cada tabla 
        ...
        //{
        //    IQueryable<TDTO> queryDto = request.Orden == "desc" ? queryable.OrderBy($"{request.Sort} descendig")
        //        : queryable.OrderBy($"{request.Sort} ascendinng");
